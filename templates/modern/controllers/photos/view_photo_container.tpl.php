<h1 class="photo-title" itemprop="name">
...
<div id="fullscreen_photo" class="<?php if ($request->isAjax()) { ?>close<?php } else { ?>disabled-act<?php } ?>"><i class="fa fa-expand"></i></div>
<?php if($prev_photo && $prev_photo['slug']){ ?>
    <a href="<?php echo  href_to('photos', $prev_photo['slug'].'.html'); ?>" class="photo_navigation prev_item" title="<?php html($prev_photo['title']); ?>"><i class="fa fa-angle-left"></i></a>
<?php } ?>
<?php if($next_photo && $next_photo['slug']){ ?>
    <a href="<?php echo  href_to('photos', $next_photo['slug'].'.html'); ?>" class="photo_navigation next_item" title="<?php html($next_photo['title']); ?>"><i class="fa fa-angle-right"></i></a>
<?php } ?>
