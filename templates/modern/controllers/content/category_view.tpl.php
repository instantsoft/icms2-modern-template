...
<?php if ($subcats && $ctype['is_cats'] && !empty($ctype['options']['is_show_cats'])){ ?>
    <div class="card p-3 mb-3 gui-panel content_categories<?php if (count($subcats)>8){ ?> categories_small<?php } ?>">
        <ul class="<?php echo $ctype['name'];?>_icon list-inline mb-0">
            ...
            ?>
                ...
                <li class="list-inline-item <?php echo str_replace('/', '-', $c['slug']);?>">
                    <i class="fa fa-folder-o text-primary"></i>
                    <a href="<?php echo href_to($base_url . (($dataset && $is_ds_view && !$is_ds_hide) ? '-'.$dataset : ''), $c['slug']); ?>">
                        <?php echo $c['title']; ?>
                    </a>
                </li>
            <?php } ?>
