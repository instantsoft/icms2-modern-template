...
<div id="comment_<?php echo $entry['id']; ?>" class="media mb-3 comment<?php if($is_selected){ ?> selected-comment<?php } ?><?php if($target_user_id == $entry['user_id']){ ?> is_topic_starter<?php } ?>" <?php if ($is_levels) { ?>style="margin-left: <?php echo $level; ?>px" data-level="<?php echo $entry['level']; ?>"<?php } ?>>
    ...
    <?php } ?>
    <?php if(!$entry['is_deleted']){ ?>
            <div <?php if (!empty($entry['user']['is_online'])){ ?>class="avatar comment_user_online mr-3" title="<?php echo LANG_ONLINE; ?>"<?php } else { ?> class="avatar mr-3"<?php } ?>>
            ...
            <div class="media-body">
                <div class="info">
                    ...
                    <?php if ($is_controls){ ?>
                        <div class="nav">
                            <a class="mr-2" href="#comment_<?php echo $entry['id']; ?>" name="comment_<?php echo $entry['id']; ?>" title="<?php html( LANG_COMMENT_ANCHOR ); ?>">#</a>
                            <?php if ($entry['parent_id']){ ?>
                                <a href="#up" class="scroll-up mr-1" onclick="return icms.comments.up(<?php echo $entry['parent_id']; ?>, <?php echo $entry['id']; ?>)" title="<?php html( LANG_COMMENT_SHOW_PARENT ); ?>">&uarr;</a>
                            <?php } ?>
                            <a href="#down" class="scroll-down" onclick="return icms.comments.down(this)" title="<?php echo html( LANG_COMMENT_SHOW_CHILD ); ?>">&darr;</a>
                        </div>
