...
function html_textarea($name='', $value='', $attributes=array()){
    ...
    $class = 'form-control';
    if (isset($attributes['class'])) { $class .= ' '.$attributes['class']; }
	$html = '<textarea rows="3" name="'.$name.'" class="'.$class.'" '.$attr_str.'>'.htmlspecialchars($value).'</textarea>';
	return $html;
}
...
function html_datepicker($name='', $value='', $attributes=array(), $datepicker = array()){
    ...
    $attr_str = html_attr_str($attributes);
	$html  = '<input type="text" placeholder="'.LANG_SELECT.'" name="'.$name.'" value="'.htmlspecialchars($value).'" class="date-input form-control mr-2"  id="'.$id.'" '.$attr_str.'/>';
