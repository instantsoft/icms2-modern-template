...
<?php if ($fields['title']['is_in_item']){ ?>
    <h1>
        ...
        <?php if ($item['is_private']) { ?>
            <span class="is_private" data-toggle="tooltip" title="<?php html(LANG_PRIVACY_HINT); ?>"><i class="fa fa-user-secret"></i></span>
        <?php } ?>
    </h1>
    <?php if ($show_bar){ ?>
        <ul class="parent_title list-inline">
            <?php if ($fields['user']['is_in_item']){ ?>
                <li class="list-inline-item album_user">
            ...
            <?php if ($fields['date_pub']['is_in_item']){ ?>
                <li class="list-inline-item album_date text-muted" title="<?php html( $fields['date_pub']['title'] ); ?>">
                    <?php if (!$item['is_pub']){ ?>
            ...
            <?php if (!empty($ctype['options']['hits_on']) && $item['hits_count']){ ?>
                <li class="list-inline-item album_hits">
                    <i class="fa fa-eye"></i>
            ...
            <?php if ($item['parent_id'] && !empty($ctype['is_in_groups'])){ ?>
                <li class="list-inline-item">
                    <i class="fa fa-folder-o text-primary"></i> <a href="<?php echo rel_to_href($item['parent_url']); ?>"><?php html($item['parent_title']); ?></a>
                </li>
            <?php } ?>
            <?php if ($is_tags){ ?>
                <li class="list-inline-item tags_bar"> <i class="fa fa-tag"></i> <?php echo html_tags_bar($item['tags']); ?></li>
            <?php } ?>
        </ul>
    <?php } ?>
